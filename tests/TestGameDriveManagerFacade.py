...
class TestGameDriveManagerFacade(unittest.TestCase):
    ...
    def _getGameDriveData(self, weeks : List[int] = None) -> List[dict]:
        ...
        data = []
        for week in weeks:
            with open(self._getTestDataPath(f"game_drive_2019_reg_{week}.json"), "rt") as fp:
...
class MockApiClient(nflapi.Client.Client):
    def __init__(self, gmdrvData : List[dict]):
        self._gmdrv_data = {}
        for r in gmdrvData:
            gsid = r["gsis_id"]
            if gsid not in self._gmdrv_data:
                self._gmdrv_data[gsid] = []
            self._gmdrv_data[gsid].append(r)
        self._req_schedules = []

    def getGameDrive(self, schedules : List[str]) -> List[dict]:
        self._req_schedules = schedules
        data = []
        for gsid in [_["gsis_id"] for _ in schedules]:
            if gsid in self._gmdrv_data:
                data.extend(self._gmdrv_data[gsid])
        return data
