...
class TestGamePlayManagerFacade(unittest.TestCase):

    def setUp(self):
        self.entityName = "game_play"
    ...
    def _getMockGamePlayManager(self, scheduleData : List[dict],
                                gmplayData : List[dict]):
        apiClient = MockApiClient(gmplayData)
        ...
        self.datamgr = GamePlayManagerFacade(self.entmgr, apiClient, schmgr, self.rostmgr)
        return self.datamgr
    ...
    def _getGamePlayData(self, weeks : List[int] = None) -> List[dict]:
        if weeks is None:
            weeks = [13, 14]
        data = []
        for week in weeks:
            with open(self._getTestDataPath(f"game_play_2019_reg_{week}.json"), "rt") as fp:
                data.extend(json.load(fp))
        return data

    def test_sync_initializes_collection(self):
        schdata = self._getScheduleData()
        gsdata = self._getGamePlayData()
        gsmgr = self._getMockGamePlayManager(schdata, gsdata)
    ...
    def test_sync_no_new_finished_noop(self):
        schdata = self._getScheduleData()
        gsdata = self._getGamePlayData([13])
        gsmgr = self._getMockGamePlayManager(schdata, gsdata)
    ...
    def test_sync_new_finished_adds(self):
        ...
        for r in schdata2:
            if not r["finished"]:
                r["finished"] = True
                reqsch2.append(r)
        gsdata1 = self._getGamePlayData([13])
        gsdata2 = self._getGamePlayData([14])
        gsmgr = self._getMockGamePlayManager(schdata1, gsdata1)
        recs1 = util.runCoroutine(gsmgr.sync())
        self.assertEqual(len(recs1), len(gsdata1), "first returned record counts differ")
        gsmgr = self._getMockGamePlayManager(schdata2, gsdata2)
        recs2 = util.runCoroutine(gsmgr.sync())
...
class MockApiClient(nflapi.Client.Client):
    def __init__(self, gmplayData : List[dict]):
        ...
        for r in gmplayData:
            gsid = r["gsis_id"]
            if gsid not in self._gmplay_data:
                self._gmplay_data[gsid] = []
            self._gmplay_data[gsid].append(r)
        self._req_schedules = []

    def getGamePlay(self, schedules : List[str]) -> List[dict]:
        self._req_schedules = schedules
        data = []
        for gsid in [_["gsis_id"] for _ in schedules]:
            if gsid in self._gmplay_data:
                data.extend(self._gmplay_data[gsid])
        return data
